...
a-space(direction="vertical" fill :size="10")
  a-form.form(:model="form")
    a-space(direction="vertical" :size="10")
      a-input(v-model="form.ocrAPI" placeholder="请先填写ComfyUI的提交 prompt API")
        template(#prefix)
          icon-robot
      .info
...
const submitJob = async () => {
  ...
  // const data = readFileSync('./assets/prompt.json', 'utf8')
  const prompt = promptData
  console.log(`prompt : ${prompt}`)

  // 飞书前端插件，是在飞书的 https 页面里打开
  // 必须走 https 的接口，无法调用 http 的接口。
  const res = await axios.post("http://localhost:8188/", {
    prompt
  })
...
const run = async () => {
  ...
  const getAttachmentUrlAndSubmitJob = async record => {
    ...
    const comfy_id = (await submitJob()).map(text => ({
      type: IOpenSegmentType.Text,
      text: text,
    }))

    table.setCellValue(form.text, record.recordId, comfy_id)
  }
